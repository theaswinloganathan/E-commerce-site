...
app.post('/api/verify-payment', async (req, res) => {
  try {
    ...
    if (!process.env.RAZORPAY_KEY_SECRET) {
      return res.status(500).json({ success: false, message: "Razorpay API keys are missing in .env file" });
    }
    ...
    if (isSignatureValid) {
      // Payment verified! In production, save order to DB here if needed
